...
export class TrendChartComponent implements OnInit {
  ...
  periodDays = [];
  groups = [{ label: 'By Status', value: true }, { label: 'By AdHoc', value: false }];
  loadDefault = true;
  chartData;
  private chart: any;
   ...
   */
  setGroups() {
    let period: any[] = ['Period'];

    let statusSubmissions: any[] = [['Success'], ['Warning'], ['Failed']];
    let statusPoints: any[] = [['Success'], ['Warning'], ['Failed']];

    let scheduleSubmissions: any[] = [['Scheduled'], ['AdHoc']];
    let schedulePoints: any[] = [['Scheduled'], ['AdHoc']];

    let xMarkers: any[] = [];

    this.periodDays.forEach(day => {
      let found = false;
      let adHocFound = false;
  ...
  setYMarkers() {
    ...
    if (this.deviationMap.info.labels.avgHist === this.deviationMap.info.labels.avgPeriod) {
      yMarkers.push({ value: this.stats.avg.period / 60, text: 'Overall Average' });
    } else {
      yMarkers.push({ value: this.stats.avg.period / 60, text: 'Period Average' });
      yMarkers.push({ value: this.stats.avg.hist / 60, text: 'Historical Average' });
    }

    if (this.stats.schedule.value) {
      yMarkers.push({ value: this.stats.schedule.value / 60, text: 'Schedule' });
    }
      ...
      tooltip: {
        format: {
          value: function(value, ratio, id, index) {
            ...
            const info = defaultView ? (data.submission.adHoc ? 'adHoc' : 'scheduled') : data.submission.status.name;
  ...
  toggleChartData() {
    if (this.loadDefault) {
      this.chart.load({
        ...
        columns: [this.chartData.statusPoints[0], this.chartData.statusPoints[1], this.chartData.statusPoints[2]]
      });
    } else {
      this.chart.load({
