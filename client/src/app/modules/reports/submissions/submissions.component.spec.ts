...
describe('SubmissionsComponent', () => {
  ...
  it('should run #ngOnInit() non-lightVersion', async () => {
    component.isLightVersion = false;

    component.ngOnInit();

    expect(component.displayType).toBe(0);
    expect(component.types[0].disabled).toBeFalsy();
  });

  it('should run #ngOnInit() lightVersion', async () => {
    component.isLightVersion = true;

    component.ngOnInit();

    expect(component.displayType).toBe(1);
    expect(component.types[0].disabled).toBeTruthy();
  });
  ...
  it('should run #getSubmissions()', async () => {
    ...
    expect(component.submissionsMapping).toHaveBeenCalledTimes(1);
  });
